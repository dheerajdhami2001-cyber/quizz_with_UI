...
The application's logic is cleanly divided into several files:

-   **`main.py`**: The entry point of the application. It initializes the question bank, the quiz logic engine (`QuizBrain`), and the user interface (`QuizInterface`).
-   **`data.py`**: Responsible for making the API call to the Open Trivia Database to fetch the quiz questions.
-   **`question_model.py`**: A simple class that defines the structure for a `Question` object (text and answer).
-   **`quiz_brain.py`**: The "engine" of the quiz. It handles the core logic, such as tracking the score, fetching the next question, and verifying the user's answer.
-   **`ui.py`**: Contains the `QuizInterface` class, which is responsible for building and managing the entire Tkinter graphical user interface, including buttons, score display, and feedback.
